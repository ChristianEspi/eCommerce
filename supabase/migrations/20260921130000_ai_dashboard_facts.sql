-- =============================================================================
-- Analista IA del dashboard (EBIM_AI_SEQUENCE, fase 02) — el DATASET reducido.
--
-- `public.ai_dashboard_facts(p_store_id)` es lo ÚNICO que ve el modelo en el
-- dashboard. Tres reglas:
--
--  1. **SECURITY INVOKER.** Cuenta exactamente lo que la RLS deja ver a quien
--     llama, igual que `dashboard_kpis`. La Edge Function la llama con el JWT
--     del usuario, nunca con `service_role`.
--  2. **Reducido.** Solo agregados y listas cortas (≤5 filas, textos ≤80
--     caracteres). Nada de tablas completas, correos, teléfonos, documentos
--     fiscales ni importes por cliente sin moneda única.
--  3. **Calculado aquí, no por el modelo.** Variaciones, antigüedades y conteos
--     salen de SQL determinista (reutiliza `analytics_kpis` y `dashboard_kpis`).
--     El modelo solo puede CITAR estas cifras por su clave; no las inventa.
--
-- Guard de rol: owner/admin, los mismos que pueden gastar cuota de la
-- funcionalidad `insights` (`ebim.ai_feature_roles`). Cada sección de un
-- módulo (inventario, entregas, cobranza) solo aparece si la sociedad lo tiene
-- contratado: la IA no habla de lo que la pantalla no enseña.
-- =============================================================================
...
comment on function public.ai_dashboard_facts(uuid) is
  'Dataset REDUCIDO del analista IA del dashboard (fase 02): agregados y listas <=5, calculados en SQL bajo la RLS de quien llama (security invoker). Owner/admin. El modelo solo cita estas cifras por clave.';
