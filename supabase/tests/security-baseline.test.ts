...
> = {
  ...
  // Cierre · relaciones y reseñas. Las dos resuelven la tienda por el slug y
  ...
  product_reviews_for_slug: {
    clase: 'publicado',
    porque: 'reseñas ya moderadas y su resumen, de un producto publicado; sin usuario ni correo',
  },
...
describe('la superficie anónima es una lista cerrada', () => {
   ...
   */
  it('el reparto por clase es 12 publicado · 7 secreto · 2 techo · 1 recogido', () => {
    const cuenta = { publicado: 0, secreto: 0, techo: 0, recogido: 0 }
    for (const entry of Object.values(ANON_SURFACE)) cuenta[entry.clase] += 1
    expect(cuenta).toEqual({ publicado: 12, secreto: 7, techo: 2, recogido: 1 })
  })
